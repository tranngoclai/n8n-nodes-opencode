...
import { isRateLimitError, isAuthError } from '../errors';
...
import { parseThinkingSSEResponse } from './sse-parser';
import type {
  AccountManagerLike,
  AnthropicRequest,
  AnthropicResponse,
        ...
        try {
            ...
            while (endpointIndex < ANTIGRAVITY_ENDPOINT_FALLBACKS.length) {
                ...
                try {
                    ...
                    logger.warn(`[CloudCode] Error at ${endpoint}:`, normalizedEndpointError.message);
            ...
            if (isNetworkError(error)) {
                ...
                if (currentFailures + 1 >= MAX_CONSECUTIVE_FAILURES) {
                    ...
                    accountManager.incrementConsecutiveFailures(account.email);
                    logger.warn(`[CloudCode] Network error for ${account.email} (${currentFailures + 1}/${MAX_CONSECUTIVE_FAILURES}), trying next account... (${normalizedError.message})`);
                }
                await sleep(1000);
                continue;
            }

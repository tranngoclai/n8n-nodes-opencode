 ...
 */
import type { RateLimitReason } from '../constants';
 ...
 */
export function parseResetTime(responseOrError: Response | Error | unknown, errorText = ''): number | null {
    ...
    if (
      ...
      'headers' in responseOrError &&
      typeof (responseOrError as { headers?: Headers }).headers?.get === 'function'
    ) {
        const headers = (responseOrError as { headers: Headers }).headers;
 ...
 */
export function parseRateLimitReason(errorText: string): RateLimitReason {

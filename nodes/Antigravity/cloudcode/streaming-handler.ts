...
import { isRateLimitError, isAuthError, isEmptyResponseError } from '../errors';
import { formatDuration, sleep, isNetworkError } from '../utils/helpers';
import { logger } from '../utils/logger';
import { getFallbackModel } from '../fallback-config';
import { parseResetTime, parseRateLimitReason } from './rate-limit-parser';
import { buildCloudCodeRequest, buildHeaders } from './request-builder';
import { streamSSEResponse } from './sse-streamer';
 ...
 */
const rateLimitStateByAccountModel = new Map<string, { consecutive429: number; lastAt: number }>(); // `${email}:${model}` -> { consecutive429, lastAt }
 ...
 */
function getDedupKey(email: string, model: string): string {
    return `${email}:${model}`;
}
 ...
 * @param {string} model - Model ID
 */
function clearRateLimitState(email: string, model: string): void {
 ...
 */
function isPermanentAuthFailure(errorText: string): boolean {
 ...
 */
function isModelCapacityExhausted(errorText: string): boolean {
    const lower = (errorText || '').toLowerCase();
 ...
 */
function calculateSmartBackoff(
  errorText: string,
  serverResetMs: number | null,
 ...
 */
function asError(error: unknown): ErrorWithMessage {
    if (error instanceof Error) return error as ErrorWithMessage;
    return new Error(String(error)) as ErrorWithMessage;
}
        ...
        try {
            ...
            while (endpointIndex < ANTIGRAVITY_ENDPOINT_FALLBACKS.length) {
                ...
                try {
                    ...
                    const normalizedEndpointError = asError(endpointError);
                    // 400 errors are client errors - re-throw immediately, don't retry
                    if (normalizedEndpointError.message.includes('400')) {
                        throw normalizedEndpointError;
                    }
                    logger.warn(`[CloudCode] Stream error at ${endpoint}:`, normalizedEndpointError.message);
                    lastError = normalizedEndpointError;
                    endpointIndex++;
                }
            }
            ...
            if (lastError) {
                if (lastError.is429) {
                    ...
                    accountManager.markRateLimited(account.email, lastError.resetMs ?? null, model);
            ...
            // Handle 5xx errors
            if (normalizedError.message.includes('API error 5') || normalizedError.message.includes('500') || normalizedError.message.includes('503')) {
            ...
            if (isNetworkError(error)) {
                ...
                if (currentFailures + 1 >= MAX_CONSECUTIVE_FAILURES) {
                    ...
                    accountManager.incrementConsecutiveFailures(account.email);
                    logger.warn(`[CloudCode] Network error for ${account.email} (stream) (${currentFailures + 1}/${MAX_CONSECUTIVE_FAILURES}), trying next account... (${normalizedError.message})`);
                }
                await sleep(1000);
                continue;
            }
 ...
 */
function* emitEmptyResponseFallback(model: string): Generator<SseEvent> {

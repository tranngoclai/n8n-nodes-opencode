...
export type SelectionStrategy = 'sticky' | 'round-robin' | 'hybrid';
...
interface AntigravityConfig {
  ...
  switchAccountDelayMs?: number;
  maxConsecutiveFailures?: number;
  extendedCooldownMs?: number;
  capacityBackoffTiersMs?: number[];
...
const typedConfig = config as AntigravityConfig | undefined;
...
export const TOKEN_REFRESH_INTERVAL_MS = typedConfig?.tokenCacheTtlMs || (5 * 60 * 1000); // From config or 5 minutes
export const REQUEST_BODY_LIMIT = typedConfig?.requestBodyLimit || '50mb';
export const ANTIGRAVITY_AUTH_PORT = 9092;
export const DEFAULT_PORT = typedConfig?.port || 8080;

// Multi-account configuration
export const ACCOUNT_CONFIG_PATH = typedConfig?.accountConfigPath || join(
    homedir(),
...
export const MAX_WAIT_BEFORE_ERROR_MS = typedConfig?.maxWaitBeforeErrorMs || 120000; // From config or 2 minutes

// Retry deduplication - prevents thundering herd on concurrent rate limits
export const RATE_LIMIT_DEDUP_WINDOW_MS = typedConfig?.rateLimitDedupWindowMs || 2000; // 2 seconds
export const RATE_LIMIT_STATE_RESET_MS = typedConfig?.rateLimitStateResetMs || 120000; // 2 minutes - reset consecutive429 after inactivity
export const FIRST_RETRY_DELAY_MS = typedConfig?.firstRetryDelayMs || 1000; // Quick 1s retry on first 429
export const SWITCH_ACCOUNT_DELAY_MS = typedConfig?.switchAccountDelayMs || 5000; // Delay before switching accounts

// Consecutive failure tracking - extended cooldown after repeated failures
export const MAX_CONSECUTIVE_FAILURES = typedConfig?.maxConsecutiveFailures || 3;
export const EXTENDED_COOLDOWN_MS = typedConfig?.extendedCooldownMs || 60000; // 1 minute
...
export const SELECTION_STRATEGIES: SelectionStrategy[] = ['sticky', 'round-robin', 'hybrid'];
export const DEFAULT_SELECTION_STRATEGY: SelectionStrategy = 'hybrid';
...
const OAUTH_CALLBACK_PORT = parseInt(globalThis.process.env.OAUTH_CALLBACK_PORT || '51121', 10);
...
export const TEST_MODELS: Record<'claude' | 'gemini', string> = {
